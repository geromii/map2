...
export default function MapChart() {
  ...
  const handleCountryClick = (countryName) => {
    ...
    dispatchTimeoutId = setTimeout(async () => {
      ...
      const resultArray = await multiplyWithScoresMatrix(
        clonedState,
        isProjectionActive,
        isSecondOrderActive,
      );
      ...
      dispatch({
        ...
        payload: { probabilities: resultArray },
      });
    }, 75);
  };
    ...
    <article class="max-w-4xl mx-auto px-5 py-8 lg:max-w-6xl lg:px-8">
      <section>
        ...
        <p class="mb-8 text-gray-600">
          ...
          <span class="text-red-800">Red side</span> (click once), and at least
          one country on the <span class="text-blue-800">Blue side</span> (click
          twice). You will then see a global opinion map.
        </p>
      </section>
      <section class="mb-8 lg:mb-12">
        ...
        <ol class="mt-2 list-decimal list-inside bg-white p-6 rounded-lg shadow space-y-3 lg:p-8">
          <li class="color-transition font-medium lg:text-lg">
            Undecided (Variable)
          </li>
          <li class="text-red-800 font-medium lg:text-lg">Side A (Dark Red)</li>
          <li class="text-blue-800 font-medium lg:text-lg">
            Side B (Dark Blue)
          </li>
          <li class="text-gray-700 font-medium lg:text-lg">
            Neutral (Dark Gray)
          </li>
        </ol>
        <p class="mt-6 text-gray-700 lg:text-lg">
      ...
      <section class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-700">
          <span class="font-bold">Geopolitics Mode</span>:
        </h2>
        <p class="text-gray-600 mt-2">
          When <span class="font-bold">Geopolitics Mode</span> is on and there
          is at least one country in each of{" "}
          <span class="text-red-800">Side A</span> and{" "}
          <span class="text-blue-800">Side B</span>, every country in the
          undecided state receives a probability of siding with either{" "}
          <span class="text-red-800">Side A</span> or{" "}
          <span class="text-blue-800">Side B</span> depending on its
          relationships with the respective sides. Each country&apos;s
          probability of siding with <span class="text-red-800">A</span> or{" "}
          <span class="text-blue-800">B</span> will be reflected by their color
          on the map.
        </p>
      </section>
      <section>
        <h2 class="text-2xl font-semibold text-gray-700">
          <span class="font-bold">War Outbreak Mode</span>:
        </h2>
        <p class="text-gray-600 mt-2">
          When <span class="font-bold">War Outbreak Mode</span> is on the
          ...
          <span class="color-transition font-medium">Germany</span> may not
          initially take a side if{" "}
          <span class="text-red-800">Saudi Arabia</span> and{" "}
          <span class="text-blue-800">Iran</span> go to war, but when all of its
          major allies side with <span class="text-red-800">Saudi Arabia</span>,
          they are much more inclined to do the same. However, if an ally is in
          the <span class="text-gray-800 font-medium">Neutral</span> state then
...
const MapControls = ({
  ...
  const updateProbabilities = async (
    newIsProjectionActive,
    newIsSecondOrderActive,
  ) => {
