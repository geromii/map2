...
function TabDemographic({ phase2Countries, phase3Countries, pageMode }) {
  ...
  const formatMoneyWithPPP = (GDP, GDP_PPP) => {
    const format = (value) => {
        const billion = value / 1e3; // Convert to billion
        return `$${billion.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`;
      
    };

    return `${format(GDP)} (${format(GDP_PPP)})b`;
  };
      ...
      <div data-pagemode={pageMode} className="w-full h-full data-[pagemode=single]:translate-y-8">
        <table className="w-full h-full">
          ...
          <tbody className='text-end text-xs md:text-sm lg:text-base'>
            ...
            <tr>
              <td style={{ width: '20%' }} className="text-blue-700 h-full">
                <div className='h-6 overflow-auto'>{phase2Countries.join(', ')}</div>
              </td>
              <td style={{ width: '20%' }} className="align-top text-right">{formatMoneyWithPPP(phase2Data.GDP, phase2Data.GDP_PPP)}</td>
            ...
            <tr data-pagemode={pageMode} className="data-[pagemode=single]:hidden">
              <td style={{ width: '20%' }} className="text-red-700 h-full">
                <div className='h-12 overflow-auto'>{phase3Countries.join(', ')}</div>
              </td>
              <td style={{ width: '20%' }} className="align-top">{formatMoneyWithPPP(phase3Data.GDP, phase3Data.GDP_PPP)}</td>
