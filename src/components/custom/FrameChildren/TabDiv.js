...
export default function TabDiv({pageMode}) {
  ...
  return (
      <Tabs defaultValue="data" className="flex flex-col items-center justify-between h-[160px] lg:h-[13.02vw]  w-full md:w-[90%] shadow-[0_5px_15px_-2px_rgba(0,0,0,0.1)] z-20 rounded-lg md:rounded-2xl mb-[-15px] lg:mb-[-30px] md:mt-[20px] bg-card/95 border-2 overflow-hidden ">
        ...
        <TabsContent value="demographics" className="flex justify-center w-full h-[160px] lg:h-[13.02vw]">
          <div className = "w-full">
            < TabDemographic />
          </div>
        </TabsContent>
        <TabsContent value="data" className="flex justify-center w-full h-[130px] lg:h-[10.02vw]">
          <TabStats pageMode = {pageMode}/>
        </TabsContent>
      </Tabs>
  );
}
